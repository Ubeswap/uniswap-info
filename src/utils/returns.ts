...
interface ReturnMetrics {
  ...
  ubeswapReturn: number // netReturn - hodlReturn
  impLoss: number
  fees: number
}
...
export function getMetricsForPositionWindow(positionT0: Position, positionT1: Position): ReturnMetrics {
  ...
  const ubeswap_return = difference_fees_usd + imp_loss_usd
...
export async function getLPReturnsOnPair(
  ...
  for (const index in snapshots) {
    ...
    ubeswapReturn = ubeswapReturn + results.ubeswapReturn
